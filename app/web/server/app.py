...
def realty_predict():
    ...
    return render_template('realty_simulator.html', prediction_text='Odhadovaná cena bytu je: {:0,.0f} Kč'.format(prediction[0]))
...
def binance_api_tool():
    ...
    if list(request.form.values()):  
        ...
        return render_template('binance_api_tool.html', jobs=jobs, form_inputs=f"Job added into the queue at {job.enqueued_at}. {q_len} tasks in the queue. {len(current_jobs)} is running.", finished_jobs=finished_jobs, current_jobs=current_jobs)
        
    return render_template('binance_api_tool.html', jobs=jobs, finished_jobs=finished_jobs, current_jobs=current_jobs, form_inputs=f"There are currently {q_len} tasks in the queue. {len(current_jobs)} is running.")
...
def enchanter_page():
    ...
    if not stop:
        ...
        return render_template('enchanter.html', ench_results=ench_results, labels=labels, column_names=df.columns.values, row_data=list(df.values.tolist()), zip=zip)
